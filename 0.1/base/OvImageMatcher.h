#ifndef __OVIMAGEMATCHER_H
#define __OVIMAGEMATCHER_H

#include "OvImageT.h"

///Abstract Base Class for matching a pair of images.
/** 
* The OvImageMatcherT class defines a basic interface for matching a pair of images.
* Subclasses can implement the interface to provide a variety of local image matchers.
*
* @author Abhijit Ogale
*/
class OvImageMatcher
{
public:

	OvImageMatcher(); /**< Default constructor with no parameters */

	virtual ~OvImageMatcher(); /**< Destructor */

	/**
	* Method template used for specifying the image pair to be matched.
	...
	*/
	virtual bool setMatchingParams(int nparams, double*params) = 0;
	...
	* @return result image if sucessful (values between 0 (no match) and 1 (match)).
	...
	* @param shiftx horizontal relative shift
	* @param shifty vertical relative shift
	* @return result image if sucessful.
...
OvImageMatcher::OvImageMatcher()
{
}

virtual OvImageMatcher::~OvImageMatcher()
{
}

#endif //__OVIMAGEMATCHER_H
